...
public class DownloadService extends Service {
    ...
    public String getDiskCacheDir(Context context) {
        if (Environment.MEDIA_MOUNTED.equals(Environment.getExternalStorageState())) {
            return Constants.getDefaultPath(context);
        }
...
//        PendingIntent.getActivity(getContext(), notifyId,intent,
//                PendingIntent.FLAG_UPDATE_CURRENT);
        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.S) {
            clickIntent = PendingIntent.getActivity(getContext(), notifyId, intent,
    ...
    private void showErrorNotification(int notifyId, String channelId, @DrawableRes int icon, CharSequence title, CharSequence content, boolean isReDownload, UpdateConfig config) {
        ...
        if (isReDownload) {//重新下载
            ...
            PendingIntent clickIntent = null;
            //PendingIntent.getService(getContext(), notifyId, intent,
            //       PendingIntent.FLAG_UPDATE_CURRENT);
            if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.S) {
                clickIntent = PendingIntent.getActivity(getContext(), notifyId, intent,
                        PendingIntent.FLAG_IMMUTABLE);
            ...
            PendingIntent clickIntent = null;
            //PendingIntent.getService(getContext(), notifyId, new Intent(),
            //PendingIntent.FLAG_UPDATE_CURRENT);
            if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.S) {
                clickIntent = PendingIntent.getActivity(getContext(), notifyId, new Intent(),
                        PendingIntent.FLAG_IMMUTABLE);
            } else {
                clickIntent = PendingIntent.getActivity(getContext(), notifyId, new Intent(),
                        PendingIntent.FLAG_UPDATE_CURRENT);
